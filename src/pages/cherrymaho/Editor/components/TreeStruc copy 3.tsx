...
const TreeStruc: React.FC<Props> = ({ curSect, callback, expandKeys, autoExpand }) => {
    ...
    const renderTree = (nodes: edialog[] | eSeg[] | epart[], parent: any) => {
        ...
        return (
            ...
            _.map(nodes, (item: edialog | eSeg | epart, idx: number) => {
                ...
                >
                    <Button onClick={(e) => { onClick(item); e.stopPropagation(); }} size="small" style={{ marginRight: 5 }} shape='circle'> S </Button>
    ...
    const mergeParent = (parent: any, idx: number) => {
        ...
        if (selectedNodes.current.length) {
            ...
            let arr: any = [];
            _.map(selectedNodes.current, (prev: any) => {
    ...
    const onSelect = (keys: string[], { node }: any) => {
        if (!node.selected) {
            ...
            selectedNodes.current = _.filter(selectedNodes.current, (s: any) => s.key != node.key);
            console.log(selectedNodes.current);
        }
        callback && callback('fresh');
    };
    return (
        <div >
            <Tree
                ...
                // @ts-ignore
                onSelect={onSelect}
                selectedKeys={_.map(selectedNodes.current, (n: any) => n.key)}
            >
